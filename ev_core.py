...
def _canon_net(nt: str) -> str:
    nt = (nt or "").strip().lower()
    if nt in {"er", "random", "erdos_renyi", "erdos-renyi"}:
        return "er"
    if nt in {"ba", "barabasi_albert", "barabasi-albert", "scale_free"}:
        return "ba"
    if nt in {"ws", "small_world", "small-world", "watts_strogatz", "watts-strogatz"}:
        return "ws"
    if nt in {"grid", "lattice"}:
        return "grid"
    raise ValueError(f"Unknown network_type={nt!r}. Use ER/WS/BA/grid (or aliases).")
...
class EVStagHuntModel(Model):
    ...
    def __init__(
        ...
        if nt == "ba":
            ...
            for drop in range(side * side - 1, n_nodes - 1, -1):
                if G.has_node(drop):
                    G.remove_node(drop)

        elif nt == "ws":
            k = 4  # average degree (must be even)
            G = nx.watts_strogatz_graph(n_nodes, k, p, seed=seed)  # <-- uses YOUR p
        ...
        #     # Build a 2D grid approximating sqrt(n_agents) × sqrt(n_agents)
        #     side = int(np.sqrt(n_nodes))
        #     # If n_agents isn't a perfect square, pad
        #     if side * side != n_nodes:
        #         side = int(np.ceil(np.sqrt(n_nodes)))
        #         pad = side * side - n_nodes
        #         # Build grid and remove excess nodes later
        #         G = nx.grid_2d_graph(side, side)
        #         # Relabel nodes to integers
        #         G = nx.convert_node_labels_to_integers(G)
        #         # Drop extra nodes if needed
        #         if pad > 0:
        #             max_node = side * side - 1
        #             for drop in range(max_node, max_node - pad, -1):
        #                 G.remove_node(drop)
        #     else:
        #         G = nx.grid_2d_graph(side, side)
        #         G = nx.convert_node_labels_to_integers(G)
        ...
        #     # k = average degree, p_rewire = rewiring probability
        #     # You can expose these as __init__ params if you like
        #     k = 4
        #     p_rewire = 0.05
        #     G = nx.watts_strogatz_graph(n_nodes, k, p_rewire)
        # else:
        #     G = nx.erdos_renyi_graph(n_nodes, p, seed=seed)
        self.G = G
...
def run_network_trial(
    ...
    *,
    ...
    if collect:
        if model.datacollector is None:
            raise RuntimeError("collect=True but model.datacollector is None")

        mdf = model.datacollector.get_model_vars_dataframe()
        return {
            "X_t": mdf["X"].to_numpy(),
            "I_t": mdf["I"].to_numpy(),
            "X_star": float(model.get_adoption_fraction()),
            "T_run": int(len(mdf)),
        }

    return float(model.get_adoption_fraction())
